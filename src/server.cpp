...
int HttpServer::setup() {
    ...
    m_sckt = socket(m_serverInfo->ai_family, m_serverInfo->ai_socktype,
...
int HttpServer::handleRequest(int clientSocket) {
    ...
    // thinking of sckt as a external guard and newSckt (clientSocket) as the
    // bartender inside the show now we use recv and send functions (they work
    // with array buffers
    std::vector<char> buffer;
    buffer.resize(1024);

    // recv takes a socket, a buffer to put data into, the maximum lenght of
    // buffer and flags flag 0 is default receives in bytes the client request
    // recv always receive the same format [METHOD] [SPACE] [PATH] [SPACE]
    // [VERSION] then header files like Host: localhost:8080, User-Agent: firefox
    ...
    if (path == "/api/user-agent") {
        ...
        size_t startPos =
            request.find(searchKey); // returns pointer to first occurance
    ...
    std::string localPath = path.substr(1); // Gets a substring starting at index
                                            // 1 (i.e, eliminates the backslash)
    ...
    std::string extension =
        localPath.substr(localPath.find_last_of(".")); // last occuerence of "."
...
int HttpServer::run() {
  // main loop
    while (true) {
        ...
        // receives sckt, only server address and a client address (where we write
        // the new IP) reinterpret_cast allows to remap bits, less safety, but
